...
import React, { useState, useRef, useEffect } from "react"
import { BotMessageSquare, X, ArrowUp, Copy, Check, Loader2, ExternalLink, AlertTriangle, Phone, Globe } from "lucide-react"
import { chatService } from "@/lib/api"
import { v4 as uuidv4 } from "uuid"
import ReactMarkdown from "react-markdown"
import remarkMath from "remark-math"
...
const HOTLINE_WA = "081230272723"
const WEBSITE_PMB = "https://pmb.istts.ac.id/"
...
export default function Widget() {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-[380px] h-[600px] bg-gray-950 border border-gray-700 rounded-3xl shadow-2xl flex flex-col overflow-hidden z-50">
          ...
          <div className="flex items-start gap-2.5 bg-amber-500/10 border-b border-amber-500/25 px-4 py-2.5 flex-shrink-0">
            <AlertTriangle className="w-3.5 h-3.5 text-amber-400 mt-0.5 flex-shrink-0" />
            <p className="text-[11px] text-amber-300/90 leading-relaxed">
              Chatbot ini masih dalam tahap <span className="font-semibold text-amber-400">uji coba</span>. Informasi bisa saja tidak akurat — selalu verifikasi ke sumber resmi.
            </p>
          </div>
            ...
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center px-4">
                ...
                <div className="mt-5 bg-gray-900 border border-gray-700 rounded-2xl px-4 py-3.5 text-left space-y-2">
                  ...
                  <a
                    ...
                    className="flex items-center gap-2 text-[12px] text-yellow-400 hover:text-yellow-300 transition-colors"
                  >
              ...
              messages.map((msg) => (
                <div key={msg.id} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : ""}`}>
                  ...
                  <div className="max-w-[80%] group">
                    <div
                      className={`px-5 py-3.5 text-[15px] leading-relaxed rounded-3xl shadow-sm ${
                        msg.role === "bot"
                          ? "bg-gray-900 border border-gray-700 rounded-tl-none text-gray-100"
                          : "bg-slate-700 text-white rounded-tr-none"
                      }`}
          ...
          <div className="p-4 pt-2 border-t border-gray-700 bg-gray-900">
            ...
            <div className="flex items-center justify-between mt-2.5 px-1">
              ...
              <p className="text-[10px] text-gray-600">Uji Coba • Guest Mode</p>
            </div>
          </div>
